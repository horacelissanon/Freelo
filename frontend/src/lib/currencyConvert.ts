// Shared (client + server) pure math for converting/summing mixed-currency
// rows. Deliberately NOT under lib/server/ (no `server-only` import): the
// Devis/Factures/Projets list pages need the exact same conversion the API
// aggregations use, and importing a server-only module from a 'use client'
